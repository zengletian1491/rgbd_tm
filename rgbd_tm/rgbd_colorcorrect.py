    ...
    def __init__(self):
        ...
        def objectiveFunction(x, cam_pairs, cameras, show=False):
            ...
            for i, camera in enumerate(cameras):
                ...
                #camera.rgb.gc_image = adjustGamma(camera.rgb.image, gamma = x[idx+1])
                #camera.rgb.lab_image = cv2.cvtColor(camera.rgb.gc_image, cv2.COLOR_BGR2LAB)


                camera.rgb.gc_image, camera.rgb.ycrcb_corrected_image = colorCorrectYCrCb(camera.rgb.ycrcb_image, y_bias=x[idx], y_scale=x[idx+1], u_bias=x[idx+2], v_bias=x[idx+3])
                #camera.rgb.gc_image, camera.rgb.lab_corrected_image = colorCorrectLAB(camera.rgb.lab_image, l_bias=x[idx], l_scale=x[idx+1], a_bias=x[idx+2], b_bias=x[idx+3])
            #print('color corrected in ' + str(time.time() - t) + ' secs')
            ...
            for i, cam_pair in enumerate(cam_pairs):
                ...
                #pixs_b_uncorrected = cam_b.rgb.ycrcb_image[pts2D_b[1,:], pts2D_b[0,:]]
                #print('classical pixs_b_uncorrected =\n' + str(pixs_b_uncorrected.shape))

                pixs_a = cam_a.rgb.ycrcb_corrected_image[pts2D_a[1,:], pts2D_a[0,:]]
                pixs_b = cam_b.rgb.ycrcb_corrected_image[pts2D_b[1,:], pts2D_b[0,:]]

                #pixs_a = cam_a.rgb.lab_corrected_image[pts2D_a[1,:], pts2D_a[0,:]]
                #pixs_b = cam_b.rgb.lab_corrected_image[pts2D_b[1,:], pts2D_b[0,:]]
                #print('pixs_a =\n' + str(pixs_a))
                #print('classical pixs_b =\n' + str(pixs_b))

                diff = pixs_a - pixs_b
                #print('diff =\n' + str(diff))

                dist = np.linalg.norm(diff, ord=2, axis=1)
                #print('dist (' + str(dist.shape) + ' = ' + str(dist))
                dist_norm = (dist / np.max(dist) * 255.0).astype(np.uint8)

                #dist_norm = dist / 2 * 255.0
                #print('dist_norm (' + str(dist_norm.shape) + ' = ' + str(dist_norm))

                
                e.append(np.linalg.norm(dist))

                #win_name = 'cam_pair_' + str(i) + '_cam ' + str(cam_pair['idx_a'])
                #cv2.namedWindow(win_name, cv2.WINDOW_NORMAL)
                #image = cam_a.rgb.image
                #for k in range(0, N):
                    #x0 = pts2D_a[0, k]
                    #y0 = pts2D_a[1, k]
                    #cv2.line(image, (x0, y0), (x0, y0), color=(0, round(dist_norm[k]), 0), thickness=4)
                #cv2.imshow(win_name, image)

                #win_name = 'cam_pair_' + str(i) + '_cam ' + str(cam_pair['idx_b'])
                #cv2.namedWindow(win_name, cv2.WINDOW_NORMAL)
                #image = cam_b.rgb.image
                #for k in range(0, N):
                    #x0 = pts2D_b[0, k]
                    #y0 = pts2D_b[1, k]
                    #cv2.line(image, (x0, y0), (x0, y0), color=(0, round(dist_norm[k]), 0), thickness=4)
            ...
            #print('classical computed error in ' + str(time.time() - t) + ' secs')
            ...
            #for cam_pair in cam_pairs:
                #pts2D_a = cam_pair['pts2D_a']
                #pts2D_b = cam_pair['pts2D_b']
                #idx_a = cam_pair['idx_a']
                #idx_b = cam_pair['idx_b']
                #cam_a = cameras[idx_a]
                #cam_b = cameras[idx_b]
                #_,N = pts2D_a.shape


                #pixs_a_uncorrected = np.reshape(cam_a.rgb.ycrcb_image[pts2D_a[1,:], pts2D_a[0,:]], (1,N,3))
                #pixs_b_uncorrected = np.reshape(cam_b.rgb.ycrcb_image[pts2D_b[1,:], pts2D_b[0,:]], (1,N,3))

                ##print('pixs_a_uncorrected =\n' + str(pixs_a_uncorrected))
                #print('pixs_b_uncorrected =\n' + str(pixs_b_uncorrected.shape))
                #print('image shape =\n' + str(cam_b.rgb.ycrcb_image.shape))
                #pixs_a = colorCorrectYCrCb(pixs_a_uncorrected, y_bias=x[idx_a], y_scale=x[idx_a+1], u_bias=x[idx_a+2], v_bias=x[idx_a+3])
                #pixs_b = colorCorrectYCrCb(pixs_b_uncorrected, y_bias=x[idx_b], y_scale=x[idx_b+1], u_bias=x[idx_b+2], v_bias=x[idx_b+3])
                ...
                #dist = np.linalg.norm(pixs_a - pixs_b)
                
                #ne.append(dist)

            #print('computed error in ' + str(time.time() - t) + ' secs')
        ...
        #--- Create first guess
        #---------------------------------------
        #x0 = [1.0] * len(self.cameras) #start with all camera gamma parameters to 1.0
        #x0 = [1.1, 0.4, 0.5, 2.6, 0.7]
        #x0 = [0.1, 1.4, 2.0, 2.0, 0.2]
        #x0 = list(((np.random.random_sample((len(self.cameras), )) -0.5)/10  + 1.0) )
        #x0 = list((np.random.random_sample((len(self.cameras), )) + 0.1 * 2))

        #x0 = [0.1, 0.9, 0.1, 0.1] * len(self.cameras) 
        x0 = [0.0, 1.0, 0.0, 0.0] * len(self.cameras) 
        #x0[0:4] = [0.0, 1.0, 0.0, 0.0]


        #x0 = [0.0, 1.0, 0.0, 0.0] * len(self.cameras) 
        #x0[4:8] = [-0.2, 1.0, 0.0, 0.2]
        print('x0 = ' + str(x0))
        ...
        #Bmin = [0.1]* len(x0) #max gamma value for all cameras
        #Bmin[0] = 0.95
        #Bmax = [3.0] * len(x0) #min gamma value for all cameras
        #Bmax[0] = 1.05
        #x0[0] = 1

        #Bounds for YCrCb
        Bmin = [-0.25, 0.0, -0.25, -0.25]* len(self.cameras) #max gamma value for all cameras
        Bmin[0:4] = [-0.0001, 0.9999, -0.0001, -0.0001]
        Bmax = [0.25, 2.0, 0.25, 0.25] * len(self.cameras) #min gamma value for all cameras
        Bmax[0:4] = [0.0001, 1.0001, 0.0001, 0.0001]
        ...
        #max_ab = 127
        #min_ab = -127
        #Bmin = [-min_l, 0.0, min_ab, min_ab]* len(self.cameras) #max gamma value for all cameras
        #Bmin[0:4] = [-0.0001, 0.9999, -0.0001, -0.0001]

        #Bmax = [max_l, 3.0, max_ab, max_ab] * len(self.cameras) #min gamma value for all cameras
        #Bmax[0:4] = [max_l, 1.0001, max_ab, max_ab]
        ...
        res = least_squares(objectiveFunction, x0, verbose=2, jac_sparsity=SM, gtol=1e-3, ftol=1e-3, xtol=1e-3, method='trf', bounds=bounds, args=(cam_pairs, self.cameras, self.p['view_optimization']), diff_step = 1e-3, x_scale='jac', tr_solver='lsmr')
